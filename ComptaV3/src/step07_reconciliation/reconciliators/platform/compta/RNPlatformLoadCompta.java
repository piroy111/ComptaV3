...
public class RNPlatformLoadCompta {

	public RNPlatformLoadCompta(RNPlatform _sRNPlatform) {
	...
	public final void run() {
		 ...
		 * Initiate
		 */
		BKPartitionPerBKAccount lBKPartitionPerAccount = pRNPlatform.getpBKPartitionManager().getpBKPartitionPerBKAccount();
		...
		for (RNPlatformDate lRNPlatformDate : pRNPlatform.getpRNPlatformDateManager().getpTreeMapDateToRNPlatformDate().values()) {
			...
			for (BKAccount lBKAccount : BKAccountManager.getpListBKAccount()) {
				 ...
				 * 
				 */
				TreeMap<Integer, BKTransactionPartitionDate> lTreeMapDateToBKTransactionPartitionDate = lBKPartitionPerAccount
							...
							if (!AMNumberTools.isNaNOrNullOrZero(lHolding)) {
								...
								if (lBKAsset instanceof BKAssetMetal) {
									...
									for (BKBar lBKBar : lBKHoldingAssetDate.getpTreeMapBKBarToHolding().keySet()) {
										...
										/*
										 * If the BKBar is outside of the platform, then we discount it from the COMPTA (because it is not counted in the platform)
										 */
										if (lRNPlatformDate.getpRNPlatformOutsideDate().getpListBKBarOutsideOfPlatform().contains(lBKBar)) {
											lRNPlatformDateAccountAsset.addpAmountCompta(-lBKBar.getpWeightOz());
										}
									}
								}
							}								
